...
Reversible workspace-write acceptance also passed natively in both Claude and ChatGPT. Each host checked the exact original name, used a fresh preview and unique idempotency key to apply a temporary QA suffix, read the saved name, then used a new preview to restore ChatGPT Plugin Review Workspace. Final readback confirmed the original name and unchanged slug/membership. ChatGPT used one-time tool approval. No invitation, role change, booking, mail or public content action was performed. The mutation response's workspace envelope retains the pre-write name; separate member-list readback confirmed actual state. This response clarity issue is a follow-up, not a failed mutation.

Widget metadata follow-up: #5213 merged as 51dcf27036d6efc4b169a9eb419d497d06a88ed8. Full post-merge test run 35930736913 and staging release 35930737754 passed. Production promotion 35931838620 succeeded through protected release PR #5214, production SHA c53e382f4d1ab357adddbd5c06e10478c59f99c8. ChatGPT tool refresh confirmed ui.domain=https://mcp.medalsocial.com and removed the missing-domain submission warning. Native readback rendered the saved draft in the new mcp-medalsocial-com sandbox; its mobile preview was visually inspected with logo, large heading and hero intact. The iframe-review notice remains expected. Separate Claude staging consent was subsequently approved and booking acceptance completed, as recorded below.


## Native Claude staging booking acceptance — 24 September 2026

The user approved the separate staging grant. Reconnected Medal Business Staging to Medal Email Connector Staging Demo (only that workspace). Native Claude verified the workspace, today's Europe/Oslo summary, synthetic service/staff catalogue, future slots, and the designated synthetic contact. Medal's own calendar independently matched the read results.

The synthetic MCP Demo Consultation costs zero NOK. Its contact has a reserved example.com email and no phone. Creation may attempt standard confirmation/reminder delivery to that test destination; suppression and delivery logs were not verified. No real customer was a target. Rescheduling used notify_customer:false.

- Created once at 30 September 2026, 09:00 Oslo: rs9bwh81tvya1c0qe3d0f0da9s8eyqpt. An identical execute retry returned the same ID.
- Moved through a fresh preview to 10:00 Oslo: replacement rs9dmbzptvfaz0v78g7h5189958ezqyq. Identical reschedule replay returned the same replacement, preserving the original as cancelled/rescheduled and the backward history link.
- Independent Medal calendar readback showed the 09:00 appointment and then its 10:00 replacement.
- Cancelled the replacement in Medal with reason Synthetic Claude MCP native acceptance completed; cleanup. Native readback confirmed both cancelled, no active appointment, and the 09:00/10:00 slots available again. Cancelled history is intentionally retained. No MCP cancellation tool is claimed: cleanup used Medal's UI.

Native evidence: https://claude.ai/chat/f2d0d570-aad7-4809-b506-fd6cee7aff96

This completes native Claude booking read/create/move/replay acceptance in the configured staging demo. It does not prove native ChatGPT booking writes, native invitation/role changes, native site/traffic edits, or live Google Business updates. The current grant is limited to the email/booking demo; the configured Medal Social staging site is a separate workspace. Existing backend staging evidence for those other operations remains separate. Booking source currently reports dashboard rather than MCP; clearer actor/source attribution is a useful follow-up.
